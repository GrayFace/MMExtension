...
static void __fastcall CallbackCall(int *params, lua_State *L)
{
	for (int i = 3, n = lua_gettop(L); i <= n; i++, params++)
		if (!ToParam(L, i, *params))
			luaL_error(L, "invalid parameter #%d passed to mem.call", i - 2);
}

static int CallMultiParam(void* callwhat, int _size, int fastcount, void* param) // fastcount: 0 - normal, 1 - thiscall, 2 - fastcall, 3 - fastcall+eax
{
	int result;
	_asm
	{
...
static int CallPtr(lua_State *L)
{
	...
	int ret = CallMultiParam((void*)ToInteger(L, 1), n*4, min(n, ToInteger(L, 2)), L);
	lua_settop(L, 0);
...
static void __fastcall Mem_HookProcCall(DWORD data)
{
	...
	lua_pushnumber(Lua, **(DWORD**)(data + 0x10) - 5);  // u4[data.esp]
	lua_call(Lua, 2, 0);
}
