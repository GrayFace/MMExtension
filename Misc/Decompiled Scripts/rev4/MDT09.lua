...
evt.map[376] = function()  -- Timer(<function>, 3.5*const.Minute)
	if evt.Cmp("QBits", 22) then         -- "Find the map that shows the location of Sir BunGleau's panoply and return it to the Massenger of the Saints.."
		if not evt.Cmp("QBits", 23) then         -- Found map when on Crusader quest
			if evt.CheckMonstersKilled{CheckType = 1, Id = 5, Count = 1} then
				evt.ForPlayer("All")
				evt.Set("QBits", 23)         -- Found map when on Crusader quest
				evt.SetNPCGreeting{NPC = 17, Greeting = 0}         -- "Sally" : ""
