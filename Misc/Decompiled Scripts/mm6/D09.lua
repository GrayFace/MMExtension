local TXT = Localize{
	...
	[15] = "Snergle's Iron Mines",
}
table.copy(TXT, evt.str, true)

Game.MapEvtLines.Count = 0  -- Deactivate all standard events

evt.MazeInfo = evt.str[15]  -- "Snergle's Iron Mines"
...
evt.map[27] = function()
	...
	if not evt.Cmp("MapVar19", 1) then
		evt.SummonMonsters{TypeIndexInMapStats = 1, Level = 1, Count = 2, X = -1280, Y = 3200, Z = 0}
		evt.SummonMonsters{TypeIndexInMapStats = 1, Level = 2, Count = 2, X = -1920, Y = 2688, Z = 0}
		evt.SummonMonsters{TypeIndexInMapStats = 1, Level = 3, Count = 2, X = -2560, Y = 2944, Z = 0}
		evt.SummonMonsters{TypeIndexInMapStats = 3, Level = 1, Count = 1, X = -1792, Y = 3584, Z = 0}
		evt.Set("MapVar19", 1)
	end
end
...
evt.map[41] = function()
	if not evt.Cmp("QBits", 1) then         --  1 D09, key to open D05
		evt.SpeakNPC(66)         -- "Ghim Hammond"
		evt.Add("Inventory", 488)         -- "Key to Snergle's Chambers"
		evt.Set("QBits", 1)         --  1 D09, key to open D05
	end
end

evt.hint[50] = evt.str[9]  -- "Exit"
evt.map[50] = function()
	evt.MoveToMap{X = 20212, Y = 13238, Z = 97, Direction = 1408, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 6, Name = "OutC3.Odm"}
end

evt.map[60] = function()  -- RefillTimer(<function>, const.Day)
	evt.Set("MapVar49", 20)
	evt.Set("MapVar50", 20)
end

RefillTimer(evt.map[60].last, const.Day)
