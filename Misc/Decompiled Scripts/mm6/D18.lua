...
evt.MazeInfo = evt.str[10]  -- "Gharik's Forge"
...
evt.map[16] = function()
	if not evt.Cmp("QBits", 11) then         -- 11 D7, opens tomb in D18.
		goto _4
	end
	if evt.Cmp("QBits", 199) then         -- Quest item bits for seer
		goto _4
	end
::_9::
	evt.SetDoorState{Id = 29, State = 1}
	evt.SetDoorState{Id = 30, State = 1}
	evt.Subtract("Inventory", 487)         -- "Key to Gharik's Laboratory"
	evt.Add("QBits", 376)         -- NPC
	evt.Subtract("QBits", 199)         -- Quest item bits for seer
	do return end
::_4::
	evt.ForPlayer("All")
	if not evt.Cmp("Inventory", 487) then         -- "Key to Gharik's Laboratory"
		if not evt.Cmp("QBits", 376) then         -- NPC
