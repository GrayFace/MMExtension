local TXT = Localize{
	...
	[16] = "Lord of Fire",
	[17] = "Hall of the Fire Lord",
}
table.copy(TXT, evt.str, true)

Game.MapEvtLines.Count = 0  -- Deactivate all standard events

evt.MazeInfo = evt.str[17]  -- "Hall of the Fire Lord"
...
evt.map[27] = function()
	evt.ForPlayer(0)
	if not evt.Cmp("Inventory", 482) then         -- "Amber"
		evt.DamagePlayer{Player = 0, DamageType = const.Damage.Phys, Damage = 50}
		evt.Set("MapVar0", 1)
	end
	evt.ForPlayer(1)
	if not evt.Cmp("Inventory", 482) then         -- "Amber"
		evt.DamagePlayer{Player = 1, DamageType = const.Damage.Phys, Damage = 50}
		evt.Set("MapVar0", 1)
	end
	evt.ForPlayer(2)
	if not evt.Cmp("Inventory", 482) then         -- "Amber"
		evt.DamagePlayer{Player = 2, DamageType = const.Damage.Phys, Damage = 50}
		evt.Set("MapVar0", 1)
	end
	evt.ForPlayer(3)
	if not evt.Cmp("Inventory", 482) then         -- "Amber"
		evt.DamagePlayer{Player = 3, DamageType = const.Damage.Phys, Damage = 50}
		evt.Set("MapVar0", 1)
	end
	if evt.Cmp("MapVar0", 1) then
		evt.StatusText(3)         -- "All must have amber.  Take life force!"
		evt.Set("MapVar0", 0)
		evt.MoveToMap{X = -1792, Y = -19, Z = 1, Direction = 1, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "0"}
	else
		evt.StatusText(9)         -- "Ah, delicious amber!"
		evt.MoveToMap{X = -1792, Y = -19, Z = 1, Direction = 1, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "0"}
	end
end
...
evt.map[42] = function()
	if evt.Cmp("MapVar10", 1) then
		evt.OpenChest(2)
	elseif evt.Cmp("QBits", 75) then         -- Lisa
		evt.OpenChest(12)
	elseif evt.Cmp("Inventory", 559) then         -- "Chest Key"
		evt.OpenChest(2)
		evt.Subtract("Inventory", 559)         -- "Chest Key"
		evt.Set("MapVar10", 1)
		evt.Set("QBits", 75)         -- Lisa
	else
		evt.StatusText(4)         -- "The chest is locked."
	end
end
...
evt.map[51] = function()
	evt.MoveToMap{X = 16387, Y = -20005, Z = 225, Direction = 512, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 6, Name = "OutD2.Odm"}
end

evt.hint[52] = evt.str[16]  -- "Lord of Fire"
evt.map[52] = function()
	evt.SetDoorState{Id = 51, State = 1}
	evt.SetDoorState{Id = 52, State = 1}
	evt.SpeakNPC(298)         -- "Lord of Fire"
...
evt.map[56] = function()  -- Timer(<function>, const.Day, 12*const.Hour, false)
	evt.Set("MapVar1", 0)
end

Timer(evt.map[56].last, const.Day, 12*const.Hour, false)

evt.map[57] = function()  -- RefillTimer(<function>, const.Day)
	evt.Set("MapVar1", 0)
end

RefillTimer(evt.map[57].last, const.Day)
