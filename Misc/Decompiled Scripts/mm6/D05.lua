...
evt.MazeInfo = evt.str[7]  -- "Snergle's Caverns"
...
evt.map[9] = function()
	if evt.Cmp("QBits", 27) then         -- 27 D05, Given when axe is returned (so door can't be opened)
		evt.StatusText(28)         -- "The door is locked"
	elseif evt.Cmp("Inventory", 488) then         -- "Key to Snergle's Chambers"
		evt.SetDoorState{Id = 9, State = 1}
		evt.Subtract("Inventory", 488)         -- "Key to Snergle's Chambers"
