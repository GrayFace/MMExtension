...
evt.map[104] = function()
	...
	if evt.Cmp("Inventory", 532) then         -- "Chalice of Gods"
		evt.ForPlayer("All")
		evt.Subtract("QBits", 82)         -- "Find and destroy the Chalice of the Gods"
		evt.Subtract("Inventory", 532)         -- "Chalice of Gods"
...
evt.map[105] = function()  -- function events.LoadMap()
	evt.ForPlayer("All")
	if not evt.Cmp("Inventory", 532) then         -- "Chalice of Gods"
		evt.Set("Eradicated", 0)
	end
end
