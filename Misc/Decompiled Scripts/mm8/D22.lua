...
evt.map[5] = function()  -- function events.LoadMap()
	...
	if evt.Cmp("QBits", 230) then         -- You have Pissed off the clerics
		if not evt.Cmp("Counter8", 1344) then
			goto _9
		end
		...
		evt.Subtract("QBits", 230)         -- You have Pissed off the clerics
	end
::_11::
...
evt.map[6] = function()  -- function events.LeaveMap()
	if evt.Cmp("QBits", 20) then         -- Allied with Temple of the Sun. Destroy the Skeleton Transformer done.
		evt.MoveNPC{NPC = 37, HouseId = 0}         -- "Oskar Tyre"
		evt.MoveNPC{NPC = 67, HouseId = 175}         -- "Oskar Tyre" -> "Council Chamber Door"
	end
end
...
evt.map[7] = function()  -- function events.LeaveMap()
	...
	if evt.Cmp("Inventory", 604) then         -- "Nightshade Brazier"
		...
		evt.Add("QBits", 230)         -- You have Pissed off the clerics
		evt.SetMonGroupBit{NPCGroup = 39, Bit = const.MonsterBits.Hostile, On = true}         -- ""
		evt.SetMonGroupBit{NPCGroup = 40, Bit = const.MonsterBits.Hostile, On = true}         -- ""
		evt.Set("QBits", 203)         -- Nightshade Brazier - I lost it
	end
end
...
evt.house[201] = 182  -- "Temple of the Sun Leader Room"
...
evt.map[451] = function()  -- Timer(<function>, 5*const.Minute)
	evt.ForPlayer("All")
	if evt.Cmp("Inventory", 604) then         -- "Nightshade Brazier"
		evt.SetDoorState{Id = 1, State = 1}
		evt.SetDoorState{Id = 3, State = 0}
...
Timer(evt.map[451].last, 5*const.Minute)
...
evt.hint[501] = evt.str[1]  -- "Leave the Temple of the Sun"
evt.map[501] = function()
	evt.MoveToMap{X = -9754, Y = -10226, Z = 1890, Direction = 0, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 1, Name = "Out07.odm"}
end
