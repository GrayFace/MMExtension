local TXT = Localize{
	[0] = " ",
	[1] = "Leave the Regnan Pirate Outpost",
	[2] = "Door",
	[3] = "Chest",
	[4] = "Lever",
	[5] = "Button",
	...
	[9] = "Bookshelf",
	[10] = "",
	[11] = "Enter the Submarine",
	[12] = "Submarine",
	[13] = "Key Hole",
	[14] = "You don't have the Key",
...
evt.map[6] = function()  -- function events.LeaveMap()
	if not evt.Cmp("QBits", 6) then         -- Pirate Leader in Dagger Wound Pirate Outpost killed (quest given at Q Bit 5). Ends pirate/lizardman war on Dagger Wound. Shuts off pirate timer.
		if evt.CheckMonstersKilled{CheckType = 4, Id = 1, Count = 0, InvisibleAsDead = 1} then
			evt.Set("Counter1", 0)
			evt.Add("QBits", 6)         -- Pirate Leader in Dagger Wound Pirate Outpost killed (quest given at Q Bit 5). Ends pirate/lizardman war on Dagger Wound. Shuts off pirate timer.
		end
	end
end
...
evt.map[7] = function()  -- function events.LeaveMap()
	evt.ForPlayer("All")
	if evt.Cmp("Inventory", 619) then         -- "Pirate Leader's Key"
		evt.Set("QBits", 214)         -- Pirate Leader's Key - I lost it
	end
end
...
evt.map[13] = function()
	...
	evt.CastSpell{Spell = 6, Mastery = const.Master, Skill = 20, FromX = 3142, FromY = -3270, FromZ = -576, ToX = 3142, ToY = 538, ToZ = -576}         -- "Fireball"
end
...
evt.hint[61] = evt.str[13]  -- "Key Hole"
evt.map[61] = function()
	evt.ForPlayer("All")
	if evt.Cmp("Inventory", 619) then         -- "Pirate Leader's Key"
		...
		evt.StatusText(14)         -- "You don't have the Key"
	end
end
...
evt.hint[401] = evt.str[12]  -- "Submarine"
evt.hint[451] = evt.str[11]  -- "Enter the Submarine"
...
evt.map[452] = function()
	...
	evt.CastSpell{Spell = 15, Mastery = const.Master, Skill = 20, FromX = 448, FromY = -608, FromZ = 632, ToX = -1, ToY = -1, ToZ = -1}         -- "Sparks"
end
...
evt.map[454] = function()
	...
	evt.CastSpell{Spell = 15, Mastery = const.Master, Skill = 20, FromX = 256, FromY = -608, FromZ = 632, ToX = -1, ToY = -1, ToZ = -1}         -- "Sparks"
end

evt.hint[501] = evt.str[1]  -- "Leave the Regnan Pirate Outpost"
evt.map[501] = function()
	evt.MoveToMap{X = 4217, Y = -17375, Z = 146, Direction = 1536, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 1, Name = "out01.odm"}
end
