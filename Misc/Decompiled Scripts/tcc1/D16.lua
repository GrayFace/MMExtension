...
evt.MazeInfo = evt.str[16]  -- "Haldegarn's Lair"
...
evt.map[6] = function()
	if not evt.Cmp("Inventory", 565) then         -- "Storage Key"
		...
		if not evt.Cmp("Inventory", 571) then         -- "Storage Key"
			evt.StatusText(7)         -- "The door is double locked."
		else
...
evt.map[50] = function()
	evt.MoveToMap{X = 20212, Y = 13238, Z = 179, Direction = 1408, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "OutC3.Odm"}
end
...
evt.map[60] = function()  -- Timer(<function>, 10*const.Minute)
	evt.ForPlayer("All")
	evt.Set("Afraid", 0)
	evt.Set("Weak", 0)
end

Timer(evt.map[60].last, 10*const.Minute)
