...
evt.MazeInfo = evt.str[11]  -- "Druid's Temple"
...
evt.map[29] = function()
	if evt.Cmp("QBits", 4) then         --  4 D10, given when you retreive artifact from chest
		evt.StatusText(13)         -- "The chest is locked."
	else
		evt.Set("QBits", 4)         --  4 D10, given when you retreive artifact from chest
		evt.StatusText(12)         -- "You open the chest and exchange the pearls."
		evt.ForPlayer("All")
		evt.Subtract("Inventory", 458)         -- "Pearl of Turmoil"
		evt.ForPlayer("Current")
		evt.Add("Inventory", 459)         -- "Pearl of Fertility"
	end
end
...
evt.map[30] = function()
	...
	if not evt.Cmp("MapVar0", 5) then
		evt.SummonMonsters{TypeIndexInMapStats = 3, Level = 2, Count = 3, X = -2976, Y = 2512, Z = 0}
		evt.SummonMonsters{TypeIndexInMapStats = 3, Level = 3, Count = 1, X = -2974, Y = 1510, Z = 0}
		evt.Add("MapVar0", 1)
	end
end
...
evt.map[31] = function()
	...
	if not evt.Cmp("MapVar0", 5) then
		evt.SummonMonsters{TypeIndexInMapStats = 3, Level = 1, Count = 3, X = -1111, Y = 4424, Z = 0}
		evt.SummonMonsters{TypeIndexInMapStats = 3, Level = 2, Count = 2, X = -1100, Y = 4420, Z = 0}
		evt.Add("MapVar0", 1)
	end
end
	...
	if not evt.Cmp("MapVar4", 1) then
		if evt.Cmp("MapVar2", 1) then
			if evt.Cmp("MapVar2", 3) then
				...
				evt.SummonMonsters{TypeIndexInMapStats = 3, Level = 3, Count = 3, X = -1152, Y = 9840, Z = -256}
				evt.SetDoorState{Id = 17, State = 1}
				evt.Set("MapVar4", 1)
			else
				evt.StatusText(8)         -- "Guards!"
				evt.SummonMonsters{TypeIndexInMapStats = 3, Level = 2, Count = 3, X = -1216, Y = 10992, Z = -256}
				evt.Add("MapVar2", 1)
			end
		end
	end
