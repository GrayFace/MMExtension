...
evt.map[37] = function()
	...
	if evt.Cmp("NPCs", 77) then         -- "Judge Fairweather"
		...
		elseif evt.Cmp("QBits", 82) then         -- Gave false plans to elfking (betray)
			if evt.Cmp("QBits", 83) then         -- Gave false Loren to Catherine (betray)
				goto _19
			end
		elseif evt.Cmp("QBits", 83) then         -- Gave false Loren to Catherine (betray)
			if evt.Cmp("QBits", 85) then         -- Gave artifact to elves
				goto _24
			end
		end
...
function events.LoadMap()
	if not evt.Cmp("QBits", 262) then         -- Time for Gobs to appear in area 2(raiding camp)
...
evt.HouseDoor(267, 488)  -- "Skinner's House"
evt.HouseDoor(268, 489)  -- "Torrent's"
...
evt.map[301] = function()
	...
	elseif evt.Cmp("QBits", 132) then         -- Butler only shows up once (area 2)
