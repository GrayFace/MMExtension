...
local function Extend(p, size, t, tend, extraSize)
	...
	BatchAdd(t, new - p)
	if tend then
		BatchAdd(tend, new - p + (NewCount - OldCount)*size)
	end
	return new
end
...
mem.autohook(mmv(0x4466F7, 0x453FD1, 0x45173B), function(d)
	...
	if mmver == 6 then
		local old = i4[0x446B5C+6]
		BatchAdd({0x446724+4, 0x446B5C+6}, NewCount - old)
		i4[0x446717+6] = NewCount
		OldCount = patched and OldCount or 100
	elseif mmver == 7 then
	...
	if mmver == 6 then
		...
		BatchAdd({0x446719, 0x446BD7, 0x446C02}, (NewCount - OldCount)*0x38) -- ignore 0x446B5E
		d.ebx = ptr
	...
	elseif mmver == 7 then
		...
		BatchAdd({0x453FEC, 0x454752, 0x4547D9, 0x4547FE}, (NewCount - OldCount)*0x44)
		BatchAdd({0x453FF0}, NewCount - OldCount)
		d.esi = ptr
		...
		BatchAdd({0x451756, 0x451EBD, 0x451F43, 0x451F68}, (NewCount - OldCount)*0x44)
		BatchAdd({0x45175A}, NewCount - OldCount)
		if not patched then
	...
	OldCount, patched = NewCount, true
