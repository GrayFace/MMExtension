...
-- mem.autohook(mmv(0x448303, 0x454C9E, 0x4524B7), |d| if mmver ~= 8 or not d.ZF then
-- 	local t = {Result = nil}
-- 	local p = mmver == 6 and d.esp + 0x168 - 0xF8 or d.edi
-- 	for i = 1, i4[p] do
-- 		t[i] = mem.pchar[p + i*4]:lower()
-- 	end
-- 	local n = #t - 2
-- 	for i = 2, n do
-- 		if (#t[i] == 1 or t[i] == 'gm') and tonumber(t[i+1]) then
-- 			n = i - 1
-- 		end
-- 	end
-- 	for i = n + 1, #t do
-- 		t[i] = nil
-- 	end
-- 	--!a{word1, word2, ...} If spell name is "Stick of Truth", the table passed to event would be !Lua[[{"stick", "of", "truth"}]]. Words are always in lower case.
-- 	events.cocalls("UnknownMonsterSpell", t)
-- 	if t.Result then
-- 		d.esi = d.esi + (n - 1)
-- 		if mmver == 6 then
-- 			u1[d.ecx + d.eax*8 + 0x22] = t.Result
-- 		else
-- 			d.eax = t.Result
-- 		end
-- 		if mmver == 8 then
